...
def write_meanface_and_eigenfaces(mean, eigenfaces, directory = None, output_size = OUTPUT_SIZE):
	...
	if (os.path.exists(directory + "/mean.bmp")):
		clean_eigenface_images_up()
		check = True

	temp = mean.reshape(output_size)
	imwrite(directory + "/mean.bmp", temp)
...
def read_meanface_and_eigenfaces(directory = None, eigenface_count = 11):
	...
	# print(directory+"/mean.bmp")
	mean = imread(directory + "/mean.bmp", IMREAD_GRAYSCALE).reshape(1, OUTPUT_SIZE[0] * OUTPUT_SIZE[1]).astype(np.float)
	# imshow("hello",mean)
	# waitKey(0)
